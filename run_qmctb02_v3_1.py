"""
QMCTB-02 FINAL (v3.1 — reproducible release)
...
N_AVG = 100
N_REALISATIONS = 20   # ↑ increased for stability
...
def simulate_sigma(frames, sigma):
    ...
    for r in range(N_REALISATIONS):
        # ✅ FIXED: seed independent of sigma
        ...
        for _ in range(frames):
            if sigma == 0:
                ...
                phi = rng.normal(0, sigma, size=(N, N))
                E = E_det * np.exp(1j * phi)
...
def main():
    ...
    print("\nQMCTB-02 FINAL (v3.1 — reproducible)\n")
    ...
    # Save dataset (NEW 🔥)
    np.savez(
        ARTIFACT_DIR / "qmctb02_dataset.npz",
        sigma=sigma_vals,
        V_mean=V_mean,
        V_std=V_std
    )
    ...
    print(f"\nSaved → {out}")
    print(f"Dataset → artifacts/qmctb02_dataset.npz")
